...
def analizar_paquetes(paquetes, ip_src, puerto_dst):
    ...
    for numero_paquete, paquete in enumerate(paquetes, start=1):
        ...
        if paquete.haslayer(IP) and paquete.haslayer(TCP):
            ...
            ip = paquete[IP]
            tcp = paquete[TCP]
            ##(◕‿−) Si la IP de origen y el puerto de destino son los introducidos por el usuario, obtenemos la información que vamos a imprimir
...
if __name__ == "__main__":
   ...
   try:
      #(◕‿−)Introduce las líneas que faltan para:
