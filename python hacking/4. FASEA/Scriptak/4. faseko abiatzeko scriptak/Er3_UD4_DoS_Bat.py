...
if __name__ == '__main__':
    try:
        ...
        #(◕‿−)Creamos el objeto "cliente_rpc" de la clase MsfRpcClient para establecer una conexión con el servidor de Metasploit Framework (MSF) a través de su API RPC (Remote Procedure Call)
        
        #(◕‿−)Indicamos el módulo  que vamos a utilizar. Será de tipo auxiliary, y su nombre es 'dos/tcp/synflood'. La variable "exploit" será de tipo AuxiliaryModule, clase de MSF que representa
        #un módulo de tipo auxiliar. Es decir, toda la información del módulo se cargará en la variable "exploit". 

        #(◕‿−)Asignamos al parámetro RHOSTS (Remote hosts) la IP de la víctima

        #(◕‿−)Lanzamos el módulo
