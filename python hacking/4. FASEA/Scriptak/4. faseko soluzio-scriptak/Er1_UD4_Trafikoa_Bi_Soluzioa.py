...
if __name__ == "__main__":
   ...
   try:
       #Creamos un objeto de la clase FileCapture que contendrá toda la información del fichero.
       captura = pyshark.FileCapture(ruta_fichero)
       #Inicializamos la lista para almacenar las conversaciones 
       conversaciones = []
       # Iteramos por cada paquete de la captura
       for paquete in captura:
           #Si se trata de un paquete "TCP"
           if paquete.transport_layer=="TCP":
              #Si el puerto e IP introducidos por el usuario coinciden con los del paquete, añadimos a la
              #lista "conversaciones" la información correspondiente al paquete 
              if direccion_ip==paquete.ip.dst and int(puerto_usuario)==int(paquete["TCP"].dstport):
                 resultado=conversacion_red(paquete)
                 conversaciones.append(resultado)
