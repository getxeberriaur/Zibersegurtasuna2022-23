...
#RESUMEN DEL SCRIPT: 
#El usuario introduce una direccióon IP y una lista de puertos; realizamos un ACK Scan a cada puerto, y se nos mostrá si está filtrado o no
#*******************************************************************************************************************************************
...
if __name__ == "__main__":   
    ...
    try:    
        ...
        for puerto in puertos_destino:
            ...
            respuesta = sr1(IP(dst=ip_destino)/TCP(sport=puerto_origen, dport=int(puerto), flags="A"), timeout=5)

            # Comprobamos si no recibimos ninguna respuesta, en cuyo caso damos el puerto por cerrado
            if str(respuesta) == "None":
                print(puerto + ": Puerto filtrado")
            ...
            # Comprobamos si la respuesta recibida es un paquete TCP
                ...
                # Comprobamos si se ha recibido un paquete de respuesta con el flag establecido en 0x04 (RST) activado, en cuyo
                # caso el puerto está filtrado
                if respuesta.getlayer(TCP).flags == 0x04:
                    print(puerto + ": Puerto no filtrado")
                else:
