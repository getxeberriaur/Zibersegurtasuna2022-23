...
if __name__ == '__main__':
    ...
    try:
        ...
        #Creamos una lista que contenga los datos formateados que nos devuelve la función formateoNmapscan y la ordenamos.
        estructuraNmap = formateoNmapScan(nm)
        estructuraNmap.sort(key=lambda x: x.host)
        #Recorremos la lista, imprimiendo la IP del host
        #Dependiendo de si nos han solicitado que mostremos los puertos abiertos o cerrados, imprimimos unos u otros.  
        for host in estructuraNmap:
            ...
            if (funcion.lower() == "a"):
                ...
                for puertoOpen in host.puertosAbiertos:  
                    print(f"{puertoOpen.puerto}/{puertoOpen.nombre}/{puertoOpen.version}")
            else:
                print ("Listado de puertos cerrados:")
                for puertoClosed in host.puertosCerradosFiltrados:  
                    print(f"{puertoClosed.puerto}/{puertoClosed.nombre}/{puertoClosed.version}")         
    except ValueError:
        print ("Error inesperado")
exit()
